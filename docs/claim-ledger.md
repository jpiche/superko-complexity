...
|---|---|---|---|---|---|
| C-1 | Encoding (C) — position as root, empty history — is the faithful formalization of SUPERKO-GO | open | prose-only | - | docs/formal-model.md §5. Pressure: both six-point positions separating the superko rules under (C) (C-56) do so by returning to a board the fresh history leaves open; the 1x6 one is reached in a game only after a White pass, after which Black wins under either rule by passing (C-58, proved by hand), and along the shortest games reaching the 2x3 one the rules agree (notebook/2026-09-13-why-the-witnesses-separate.md, computed); experiment 006 |
...
| C-58 | In a game under either superko rule, at a situation whose board carries a stone and every stone is the mover's, the last move was a pass. On 1x6 from the empty board, X.X.X. with Black to move is reached only after a White pass with the pass count one, and Black then wins at every komi floor below 6 under either rule by passing | proved | formalizable | - | proofs/C-58.md, by hand: the one step about plays is Basic.resolve_self, kernel-checked; the argument over a game is checked by no kernel. One reaching game is replayed and solved under both rules in crates/superko-solve/tests/lines.rs (computed). Contrast C-56: under encoding (C) the same position is worth 1 under PSK and 6 under SSK |
