...
| Finding | Consequence |
|---|---|
| Any of the five counts differs under the no-suicide rule | The definitions are wrong, the source is wrong, or the mirror is wrong. Stop. Diff the mirror against `Defs.lean` first, then the small-board game graph against the paper's Figures 1 and 2 (5 nodes and 12 edges on 1×2, 15 and 42 on 1×3). Nothing else proceeds until the cause is named. |
...
4. Cross-check the small-board game graphs against Figures 1 and 2 of the
   paper, as a second, independent quantity.
